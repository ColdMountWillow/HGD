#The network_GramGAN file used during testing.
import torch
import time
        ...
        if not encode_only:
            ...
            #print("i:",i)
                ...
                if i + 1 == layers[-1] and encode_only:
                    return feats,feats2  # 返回编码器部分的特征图
        # out = self.UpBlock2(x)
...
class ResnetGramGANBlock(nn.Module):
    def __init__(self, dim, use_bias):
        ...
        self.norm1 = GramLIN(dim)    #ResnetGramGANBlock 是 ResNet 块的变体，包含 Gram LIN 层。
...
class GramLIN(nn.Module):
    ...
    def forward(self,input,target_label,device):    
        ...
        target_label =target_label.to(input_device)
        ...
        theta = torch.mm(gram, self.weight_1).to(input_device)  # 确保 theta 在 input_device 上
