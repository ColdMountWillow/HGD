...
class PatientDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        graph = torch.from_numpy(graph/255).permute((2, 0, 1)).float()
        ...
        if index % NUM_FLIPS_ROTATIONS == 0 or not self.transformations:
            ...
            return patch.flip(2), graph.flip(2)
        elif index % NUM_FLIPS_ROTATIONS == 2:
            return patch.flip(1), graph.flip(1)
        elif index % NUM_FLIPS_ROTATIONS == 3:
            return patch.flip(1).flip(2), graph.flip(1).flip(2)
        elif index % NUM_FLIPS_ROTATIONS == 4:
            return patch.transpose(1, 2), graph.transpose(1, 2)
        elif index % NUM_FLIPS_ROTATIONS == 5:
            return patch.transpose(1, 2).flip(2), graph.transpose(1, 2).flip(2)
        elif index % NUM_FLIPS_ROTATIONS == 6:
            return patch.transpose(1, 2).flip(1), graph.transpose(1, 2).flip(1)
        else:
            return patch.transpose(1, 2).flip(1).flip(2), graph.transpose(1, 2).flip(1).flip(2)
