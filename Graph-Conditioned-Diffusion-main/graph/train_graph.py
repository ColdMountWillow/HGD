...
from patient_dataset_graph import PatientDataset
import os
import pandas as pd
...
def main():
    ...
    trainer = ImagenTrainer(
        imagen=imagen,
        dl_tuple_output_keywords_names=('images', 'cond_images'),
        fp16=True,
    )
    ...
    dataset = PatientDataset(data_path, patch_size=1024, image_size=1024)
    ...
    for i in range(10):
        patch, labelmap = train_dataset[i]
    ...
    while True:
        ...
        if not (step_num % args.sample_freq):
            ...
            if trainer.is_main:
                lowres_image, lowres_labelmap = dataset[0]
                ...
                with torch.no_grad():
                    images = trainer.sample(
                        ...
                        cond_images=torch.stack([lowres_labelmap, rand_labelmap]),
                    )
...
def parse_args():
    ...
    parser.add_argument('--unet1_checkpoint', type=str, default='checkpoints_graph/unet1_checkpoint.pt', help='Path to checkpoint for unet1 model')
    parser.add_argument('--unet2_checkpoint', type=str, default='checkpoints_graph/unet2_checkpoint.pt', help='Path to checkpoint for unet2 model')
    parser.add_argument('--unet3_checkpoint', type=str, default='checkpoints_graph/unet3_checkpoint.pt', help='Path to checkpoint for unet3 model')  
