...
def concatenate_ordered_subgraphs(subgraph_matrices1, subgraph_feature_vectors1, subgraph_matrices2, subgraph_feature_vectors2):
    ...
    # Determine how many subgraphs to select in total, ensuring the number is up to 3
    total_to_select = random.randint(1, 3)
    
    # Determine how many subgraphs to select from each set, ensuring the total does not exceed 3
    # It also allows for the possibility of selecting all from one set and none from the other
    if total_to_select == 1:
        if random.random() < 0.5:  # Randomly decide which set to select from
            selected_indices1 = random.sample(range(len(subgraph_matrices1)), 1)
            selected_indices2 = []
        else:
            selected_indices1 = []
            selected_indices2 = random.sample(range(len(subgraph_matrices2)), 1)
    else:
        selected_from_first = random.randint(0, total_to_select)  # This could be 0, meaning all are selected from the second set
        selected_indices1 = random.sample(range(len(subgraph_matrices1)), min(selected_from_first, len(subgraph_matrices1)))
        selected_indices2 = random.sample(range(len(subgraph_matrices2)), min(total_to_select - selected_from_first, len(subgraph_matrices2)))
    
    # Combine and tag the indices, then sort by origin set to ensure order
    ...
    for idx, origin in combined_indices:
        ...
        if final_adj_matrix is None:
            ...
            # Concatenate matrices and feature vectors with necessary adjustments for zero padding
            final_adj_matrix, final_features = adjust_and_concatenate(final_adj_matrix, final_features, matrix, features)
    
    return final_adj_matrix, final_features

# Helper function to concatenate matrices and feature vectors with padding
def adjust_and_concatenate(final_adj_matrix, final_features, matrix, features):
...
output_dir = path/to/generated_graphs/partial_extracted_short
num_graphs = 10000
