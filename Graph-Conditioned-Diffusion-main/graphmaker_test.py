...
def process_image(index, output_path):
    # Load data
    image, label_mask = open_id(h5_path, index)
    ...
    segment_folder = os.path.join(output_path, str(index))
...
def read_ids(filepath):
    # Since the structure is known and fixed, we return a range of indices
    # corresponding to the number of images/labels in the dataset.
    with h5py.File(filepath, 'r') as h5:
        num_samples = h5['data'].shape[0]  # Assuming 'data' is the path for images
    return range(num_samples)


def open_id(filepath, index):
    # Access the image and label using the provided index
    with h5py.File(filepath, 'r') as h5:  
        img = np.array(h5['data'][index])  # Access the RGB image at the given index

        labelmap = np.array(h5['labels'][index])  # Access the label map at the given index
...
def main(base_output_folder):
    ...
    for index in ids:
        ...
        adjacency_matrix, overlay_image, segment_folder = process_image(index, specific_output_folder)
        '''
        try:
